...
private:
    ...
    array<IndexedStaticMesh3D<u32>, 3> m_models;
...
public:
    Application(const WindowAppConfig& config)
        ...
        m_models({
            IndexedStaticMesh3D<u32>(context(), GeneratedResources::Sphere(10)),
            IndexedStaticMesh3D<u32>(context(), GeneratedResources::Cylinder(10)),
            IndexedStaticMesh3D<u32>(context(), GeneratedResources::Cone(10))
        })
    {
    ...
    void RunIter(f64 lastIterTime, f64 lastFrameTime) override {
        ...
        // draw iterations
        {
            ...
            m_models[m_selectedModel].Draw(context(), framebufferBinder, programBinder);
        }
        // show light
        if(m_showLight) 
        {
        ...
        // draw ui
        {
            ...
            ImGui::Combo("model", &m_selectedModel, model_types, IM_ARRAYSIZE(model_types));
