...
public:
    ...
    void RunFrame(f64 lastIterTime, f64 lastFrameTime) override {
        ...
        // draw ui
        {
            ...
            ImGui::Begin("adaptive frames test settings");

            match(GetExecutionType(),
                [&](const ExecutionTypes::VsyncFrames& et) {
                    ImGui::Text("vsync frames");
                },
                [&](const ExecutionTypes::LimitedFrames& et) {
                    ImGui::Text("limited frames %.1f", et.maxFPS());
                },
                [&](const ExecutionTypes::AdaptiveVsyncFrames& et) {
                    ImGui::Text("adaptive vsync frames");
                },
                [&](const auto& other) {
